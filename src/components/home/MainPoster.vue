...
<template>
  <div class="flex flex-col space-y-2 pt-28 pb-32 md:pb-32 md:pt-32">
    ...
    <div class="px-4 md:pb-4">
      <h2 class="text-white text-2xl md:text-5xl">
        {{
          selectedMovie?.name ||
          selectedMovie?.original_ ||
          selectedMovie?.title ||
    ...
    <div
      class="max-w-xs text-xs line-clamp-5 text-shadow-md px-4 md:max-w-lg md:text-lg lg:max-w-2xl lg:text-2xl"
    >
    ...
    <div class="flex space-x-3 pt-2 px-4 md:pt-1">
      <button class="btn bg-white text-black hover:bg-red-600">
        <PlayIcon class="w-5 h-5 md:w-8 md:h-8" /> Play
      </button>
      <button class="btn bg-[gray]/90 hover:bg-gray-500">
        <ExclamationCircleIcon class="w-6 h-6 md:w-8 md:h-8" /> More Info
      </button>
    </div>
  </div>
</template>
